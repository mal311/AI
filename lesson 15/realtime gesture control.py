...
while True:
    ...
    #Define te range for skin color in HSV
    ...
    #Find countours (hand shape) in the masked image
    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    #If contours are found draw them
    if contours:
        max_contours = max(contours, key=cv2.contourArea) #Get largest contour

        if cv2.contourArea(max_contours) > 500: #Ignore small countours
