...
while True:
    ...
    filtered_img = apply_filter(img, current_filter)

    
    if current_filter == 'GRAYSCALE':
        cv2.imshow("Filtered Webcam", cv2.cvtColor(filtered_img, cv2.COLOR_GRAY2BGR))
    else:
        cv2.imshow("Filtered Webcam", filtered_img)
        ...
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            if abs(thumb_x - index_x) < 30 and abs(thumb_y - index_y) < 30:
                if current_time - last_action_time > debounce_time:
                    cv2.putText(img, "Picture captures!", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                    last_action_time = current_time
                    cv2.imwrite(f"lesson 18/picture_{int(time.time())}.jpg", img)
                    print("Picture saved")
